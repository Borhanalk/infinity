    ...
    <main className="min-h-screen bg-background text-foreground pt-16 sm:pt-20 lg:pt-24 overflow-x-hidden">
      <div className="container mx-auto px-3 sm:px-4 lg:px-6 py-6 sm:py-8 lg:py-12 xl:py-16 overflow-hidden">
        ...
        <div className="mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
          <span className="text-[#D4AF37] text-xs sm:text-sm tracking-wider uppercase block mb-2 sm:mb-3 lg:mb-4 font-bold">מוצרים</span>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl 2xl:text-7xl font-black mb-3 sm:mb-4 lg:mb-6 bg-gradient-to-r from-foreground via-[#D4AF37] to-foreground/70 bg-clip-text text-transparent">כל המוצרים</h1>
        </div>

        {/* Filters */}
        <Card className="mb-6 sm:mb-8 lg:mb-12 border-border">
        ...
        {loading ? (
          ...
          <Card className="text-center py-12 sm:py-20">
            <CardContent>
              ...
              {error && !error.includes("Database connection") && !error.includes("قاعدة البيانات") && (
                <p className="text-sm text-muted-foreground/70 mt-2">נסה מונחי חיפוש שונים או בחר קטגוריה אחרת</p>
              )}
            </CardContent>
          </Card>
        )}
